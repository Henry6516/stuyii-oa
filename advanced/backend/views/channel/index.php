...
<div class="channel-index" style="">
    <div class="row">
        ...
        <div class="col-sm-2 ">
            <input type="text" class="form-control goodscode-input" placeholder="请选择产品或填写产品编码(多个用逗号隔开)">
        </div>
        <div class="col-sm-1">
...
$('.export-joom').on('click',function() {
    ...
    var pids = $('#chanel-table').yiiGridView("getSelectedRows");
    var goodscode = $('.goodscode-input').val();
    if( (pids===null ||pids===undefined ||pids.length===0) && !goodscode) {
        alert("请选择产品或填写产品编码！");
        return false;
    }
    window.location.href = '{$joomUrl}' + '?pids=' + pids + '&suffix=' + joom + '&goodscode=' + goodscode;
})
//批量标记Wish已完善
